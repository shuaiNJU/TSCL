...
# from models.resnet_2048 import ResNet18, ResNet34,ResNet50
...
#     for fpr_level in [0.0, 0.001, 0.01, 0.1]:  # [0.0001, 0.001, 0.01, 0.1]
...
#         acc_str = 'ACC@FPR%s: overall_acc:%.4f (many_acc:%.4f, median_acc:%.4f, low_acc:%.4f | head_acc:%.4f, tail_acc:%.4f)\n' % (fpr_level, overall_acc, many_acc, median_acc, low_acc, head_acc, tail_acc)
#         print(acc_str)
#         fp.write(acc_str)
#         fp.flush()
#     fp.write('\n')
#     for tpr_level in [0.95]:  # [0.98, 0.95, 0.90, 0.80]
...
#         overall_acc = np.mean(in_preds[mask]==in_labels[mask])
#         many_acc, median_acc, low_acc, _ = shot_acc(in_preds[mask], in_labels[mask], img_num_per_cls, acc_per_cls=True)
#         acc_str = 'ACC@TPR%s: overall_acc:%.4f (many_acc:%.4f, median_acc:%.4f, low_acc:%.4f)\n' % (tpr_level, overall_acc, many_acc, median_acc, low_acc)
#         print(acc_str)
#         fp.write(acc_str)
#         fp.flush()
#     fp.write('\n')
...
def val_cifar(num_classes):
    ...
    with ExitStack() as stack:
        ...
        for images, targets in test_loader:
            images, targets = images.cuda(), targets.cuda()
            logits, scores = get_scores_fn(model, images)  # logits=[bs,11]
            probs = F.softmax(logits, dim=1)
            pred = logits[:,0:num_classes].data.max(1)[1]
            acc = pred.eq(targets.data).float().mean()
            # append loss:
    ...
    clean_str = 'ACC: test_acc:%.4f (many_acc:%.4f, median_acc:%.4f, low_acc:%.4f)' % (test_acc, many_acc, median_acc, low_acc)
    print(clean_str)
    fp.write(clean_str + '\n')
    fp.flush()
    ...
    print('ood_scores.shape:', ood_scores.shape)
    fake_ood_scores = ood_scores[sc_labels>=0]  # sc_labels>=0的OOD样本作为ID样本
    real_ood_scores = ood_scores[sc_labels<0]  # sc_labels小于0的为真实的OOD样本
    real_in_scores = np.concatenate([in_scores, fake_ood_scores], axis=0)
    print('fake_ood_scores.shape:', fake_ood_scores.shape)
    print('real_in_scores.shape:', real_in_scores.shape)
    print('real_ood_scores.shape:', real_ood_scores.shape)
    ...
    for fpr_level in [0.0, 0.001, 0.01, 0.1]:  # [0.0001, 0.001, 0.01, 0.1]
        ...
        head_acc = np.mean(acc_each_class[0:int(0.5*num_classes)])
        tail_acc = np.mean(acc_each_class[int(0.5*num_classes):int(num_classes)])
        acc_str = 'ACC@FPR%s:  acc_at_fpr_level:%.4f (many_ac:%.4f, median_acc:%.4f, low_acc:%.4f | head_acc:%.4f, tail_acc:%.4f)\n' % (
    ...
    for tpr_level in [0.95]:  # [0.98, 0.95, 0.90, 0.80]
        ...
        many_acc, median_acc, low_acc, _ = shot_acc(in_preds_above_score_threshold, in_labels_above_score_threshold, img_num_per_cls, acc_per_cls=True)
        acc_str = 'ACC@TPR%s: acc_at_tpr_level:%.4f (many_acc:%.4f, median_acc:%.4f, low_acc:%.4f)\n' % (tpr_level, acc_at_tpr_level, many_acc, median_acc, low_acc)
        print(acc_str)
        fp.write(acc_str)
        fp.flush()
        # save mask:
    ...
    for tpr_level in [0.95]: # [0.98, 0.95, 0.90, 0.80]
        ...
        acc_str = 'FPR@TPR%s: fpr:%.4f (many_rejection_ratio:%.4f, median_rejection_ratio:%.4f, low_rejection_ratio:%.4f | head_rejection_ratio:%.4f, tail_rejection_ratio:%.4f)\n' % (tpr_level, fpr[cutoff], 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root_path', '--drp', default='/root/long_tailed_ood/datasets/', help='Where you save all your datasets.')
    parser.add_argument('--dout', default='cifar', choices=['texture','svhn', 'cifar',  'tin', 'lsun', 'places365'], help='which dout to use')
    parser.add_argument('--model', '--md', default='ResNet18', choices=['ResNet18', 'ResNet34', 'ResNet50','WRN40'], help='which model to use')
    # 
    ...
    parser.add_argument('--ckpt_path', default='Results/LT_OOD_results/add_tail_class_centers_cifar100_T=0.1_ablation_rho/cifar100-0.05-OOD30000/ResNet18/e200-b256-adam-lr0.001-wd0.0005-cos_Lambda1.0-Lambda20.1-T0.1-sign-k0.6-seed152')
    ...
    if args.only_id:
        pretrained_path = os.path.join(args.ckpt_path, os.pardir) # os.pardir() 获取当前目录的父目录（上一级目录），以字符串形式显示目录名
        
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu

    save_dir = os.path.join(args.ckpt_path, 'OOD_test_Results_msp')
    create_dir(save_dir)
    ...
    if args.dataset == 'cifar10':
        ...
        #  test_set中imbalance_ratio=1,即表示test set是类均衡的，即img_num_per_cls= [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000]
    ...
    # load model:
    if args.ckpt == 'latest':
        ckpt = torch.load(os.path.join(args.ckpt_path, 'latest.pth'))['model']   # .CIFAR10-LT-stage1.pth
    else:
    ...
    # if args.only_id: # 'If true, only test ID acc'
    #     val_cifar_only_id()
    # else:   # test OOD detection performance
    val_cifar(num_classes)
