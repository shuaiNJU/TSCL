...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, pooling='avgpool', norm=nn.BatchNorm2d, return_features=False):
        ...
        self.layer4 = self._make_layer(block, 2048, num_blocks[3], stride=2, norm=norm)
        self.linear = nn.Linear(2048, num_classes)

        ####### for BCL loss #################
        # self.fc = NormedLinear(512, num_classes)
        self.head = nn.Sequential(nn.Linear(2048, 2048), nn.BatchNorm1d(2048), nn.ReLU(inplace=True),
                                      nn.Linear(2048, 128))  # cider中使用的feat_dim=128
        # self.aux_linear = nn.Linear(512, num_classes)

        self.projection = nn.Sequential(nn.Linear(2048, 2048), nn.ReLU(), nn.Linear(2048, 128))  # MLP即two layer non-linear projection head
    ...
    def forward_features(self, x):
        ...
        # print('c1.shape=',c1.shape)    # c1.shape= torch.Size([768, 64, 32, 32])
        h1 = self.layer1(c1) # (64,32,32)
        # print('h1.shape=',h1.shape)    #h1.shape= torch.Size([768, 64, 32, 32])
        h2 = self.layer2(h1) # (128,16,16)
        # print('h2.shape=',h2.shape)  # h2.shape= torch.Size([768, 128, 16, 16])
        h3 = self.layer3(h2) # (256,8,8)
        # print('h3.shape=',h3.shape)   # h3.shape= torch.Size([768, 256, 8, 8])
        h4 = self.layer4(h3) # (512,4,4) 
        # print('h4.shape=',h4.shape)    # h4.shape= torch.Size([768, 512, 4, 4]
        p4 = self.pooling(h4) # (512,1,1) 
        # print('p4.shape=',p4.shape)   # p4.shape= torch.Size([768, 512, 1, 1])
        p4 = p4.view(p4.size(0), -1) # (512)   
        # print('p4.shape=',p4.shape)    # p4.shape= torch.Size([768, 512])
        return p4
    ...
    def forward(self, x):
        p4 = self.forward_features(x)   # p4.shape= torch.Size([768, 512])
        logits = self.forward_classifier(p4)
        # print('logits.shape=',logits.shape)   # logits.shape= torch.Size([768, 10])
    ...
    #     projected_f = self.projection(p4) #projected_f.shape= torch.Size([1024, 128])
    #     # print('projected_f.shape=',projected_f.shape)  
    ...
    def forward_projection(self, x):
            ...
            ############# 为了BCL loss 加的###########
            p4 = self.forward_features(x)
            feat_mlp = F.normalize(self.head(p4), dim=1) 
            # print('feat_mlp.shape=',feat_mlp.shape)  # feat_mlp.shape= torch.Size([768, 128])
            # logits = self.fc(p4)
            logits = self.linear(p4)
            # print('self.linear.weight.T.shape=',self.linear.weight.shape)  # self.linear.weight.T.shape= torch.Size([512, 11])
            centers_logits = F.normalize(self.head(self.linear.weight), dim=1)
            # print('centers_logits.shape=',centers_logits.shape)  # centers_logits.shape= torch.Size([11, 128]
            # return projected_f
...
def ResNet50(num_classes=10, pooling='avgpool', norm=nn.BatchNorm2d, return_features=False):
    '''
    GFLOPS: 1.1635, model size: 21.2859MB
    '''
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, pooling=pooling, norm=norm, return_features=return_features)
